...
def _find_element_with_get_attribute(*, driver:WebDriver, by, value: str, attribute: str) -> Optional[str]:
...
def _get_review_info(*, driver: WebDriver, by, value_rate: str, value_cnt: str) -> Tuple[str, str]:
    primitive_review_rate = _find_element_with_text(driver=driver, by=by, value=value_rate)
    review_rate = float(primitive_review_rate) if primitive_review_rate else 0
    primitive_review_cnt = _find_element_with_text(driver=driver, by=by, value=value_cnt)
    review_cnt_compile = re.compile(r'(?<=\n).+(?=건)')
    review_cnt = int(review_cnt_compile.search(primitive_review_cnt).group()) if primitive_review_cnt else 0
...
def kakaomap_crawling(search_name:str):
    ...
    with webdriver.Chrome('/home/pangyooldonev2/chromedriver', chrome_options=chrome_options) as driver:
        ...
        check_value = _find_element_with_text(
            driver=driver,
            by='xpath',
            value='//*[@id="info.search.place.list"]/li[1]'
        )
        ...
        return address_origin, address_road_name, store_name, review_rate, review_cnt, url
