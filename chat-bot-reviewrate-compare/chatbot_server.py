...
def _check_store_exist(*, json_address: str,
                       file_name: str, 
                       sheet_name='시트1',  
                       check_value: str) -> bool:
    sheet_df = _sheet_to_df(json_address=json_address, file_name=file_name, sheet_name=sheet_name)
    return check_value in sheet_df['가게명'].tolist()    

# 시트를 업데이트 합니다.
def _sheet_update(*, json_address: str, file_name: str, sheet_name='시트1', update_list:list):
    google_client = _get_google_client(json_address=json_address)
    sheet = google_client.open(file_name).worksheet(sheet_name)
...
def _get_percentile_value(csv_name: str, get_value):
    temp_df = pd.DataFrame(pd.read_csv(csv_name).iloc[:, 0].tolist() + [get_value], columns=['temp_col'])
...
def get_info_from_kakao(search_word: str):
    ...
    if review_cnt == 0 or review_rate ==0:
        ...
        pass
    cnt, percentile_review_cnt = _get_percentile_value(csv_name='kakao_review_table_latest_review_cnt.csv', 
                                                       get_value=review_cnt
                                                      )
    _, percentile_review_rate = _get_percentile_value(csv_name='kakao_review_table_latest_review_rate.csv', 
                                                      get_value=review_rate
                                                      )
    info_list = [platform, cnt, address, review_rate, percentile_review_rate, review_cnt, percentile_review_cnt, url]

    # 만약 버퍼테이블에 검색 결과가 없는 경우 버퍼테이블에 검색 결과를 추가합니다.
    if _check_store_exist(json_address=json_address, file_name='buffer', check_value=store_name) == False: 
        _sheet_update(json_address=json_address, file_name='buffer', update_list=kakao_list)
...
def get_info_from_naver(search_word: str):   
    ...
    if review_cnt == 0 or review_rate ==0:
        ...
        pass
    cnt, percentile_review_cnt = _get_percentile_value(csv_name='naver_review_table_latest_review_cnt.csv', 
                                                       get_value=review_cnt
                                                      )
    _, percentile_review_rate = _get_percentile_value(csv_name='naver_review_table_latest_review_rate.csv', 
                                                      get_value=review_rate
                                                      )    
    info_list = [platform, cnt, address, review_rate, percentile_review_rate, review_cnt, percentile_review_cnt, url]
    
    if _check_store_exist(json_address=json_address, file_name='buffer', check_value=store_name) == False: 
        _sheet_update(json_address=json_address, file_name='buffer', update_list=naver_list)    
...
def get_info_from_google(search_word: str):
    ...
    if review_cnt == 0 or review_rate ==0:
        ...
        pass
    cnt, percentile_review_cnt = _get_percentile_value(csv_name='google_review_table_latest_review_cnt.csv', 
                                                       get_value=review_cnt
                                                      )
    _, percentile_review_rate = _get_percentile_value(csv_name='google_review_table_latest_review_rate.csv', 
                                                      get_value=review_rate
                                                      )    
    info_list = [platform, cnt, address, review_rate, percentile_review_rate, review_cnt, percentile_review_cnt, url]    
    
    if _check_store_exist(json_address=json_address, file_name='buffer', check_value=store_name) == False: 
        _sheet_update(json_address=json_address, file_name='buffer', update_list=google_list)        
...
def _message_handler(update, context):    
    ...
    sample_df = pd.DataFrame(data=info_list, 
    ...
    if len(info_list) == 0:
        ...
        df_with_style = (sample_df.style.format({'평점-상위N%':'{:,.1%}'.format, 
                                                 '리뷰수-상위N%':'{:,.1%}'.format,
                                                 '평점':'{:,.3}'.format,})
                                        .set_properties(**{'text-align': 'left', 'width':'80px'})
                                        .bar(subset=['평점-상위N%', '리뷰수-상위N%'], 
                                             width=100, 
                                             align='left', 
                                             vmin=1, 
                                             vmax=0, 
                                        ...
                                        .set_table_styles(
                                            [{"selector": "thead", "props": [("background-color", "dodgerblue"), ("color", "white")]},
                                             {"selector": "tbody td", "props": [("border", "1px solid grey"), ('font-size', '20px')]},
                                             {"selector": "th", "props": [("border", "1px solid grey"), ('font-size', '20px')]}]))
        # 챗봇으로 이미지를 전송합니다.
        dfi.export(df_with_style, 'search_img_to_bot.png')
        update.message.reply_photo(photo=open('search_img_to_bot.png', 'rb'), 
                                   caption=f"{search_word_replace}에 대한 검색 결과입니다."
                                  )
        
        for url, platform in zip(url_list, sample_df.index.tolist()):
            update.message.reply_text(text=f"""<a href='{url}'>{search_word_replace} {platform}</a>""",
